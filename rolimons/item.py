...
class item:
    ...
    def __init__(self, id: int, itemdetails: dict = None) -> None:
    ...
    def get_ownership_data(self) -> Ownership:
        request = requests.get(f'https://www.rolimons.com/item/{self.id}')
        soup = BeautifulSoup(request.text, 'html.parser')
        tags = [tag.get_text() for tag in soup.find_all(
    ...
    def get_owner_data(self) -> ItemOwners:
